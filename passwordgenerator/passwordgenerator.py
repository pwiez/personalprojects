...
class PasswordApp:
    def __init__(self, root):
        ...
        self.numbers_checkbox = tk.Checkbutton(self.root, text="Include numbers", variable=self.numbers_flag, onvalue=True, offvalue=False)
        self.lowercase_letters_checkbox = tk.Checkbutton(self.root, text="Include lowercase letters", variable=self.lowercase_letters_flag, onvalue=True, offvalue=False)
        self.uppercase_letters_checkbox = tk.Checkbutton(self.root, text="Include uppercase letters", variable=self.uppercase_letters_flag, onvalue=True, offvalue=False)
        self.symbols_checkbox = tk.Checkbutton(self.root, text="Include symbols", variable=self.symbols_flag, onvalue=True, offvalue=False)
        ...
        self.password_length_scale = tk.Scale(self.root, length=230, from_=8, to=64, orient="horizontal", variable=self.password_length)
        ...
        self.result_label = tk.Label(self.root, bg="white", padx=10, text="Your password will appear here.")
        self.result_label.place(relx=0.5, y=340, anchor="center")

        # configuring the button for copying the result to the clipboard
        self.copy_button = tk.Button(self.root, state=tk.DISABLED, text="Copy password to clipboard", command= lambda : pyperclip.copy(self.result_label.cget("text")))
        self.copy_button.place(relx=0.5, y=385, width=220, height=30, anchor="center")

        # configuring the exit button
        self.exit_button = tk.Button(self.root, text="Exit", command=root.destroy)
